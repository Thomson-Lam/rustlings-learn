...
fn build_scores_table(results: &str) -> HashMap<&str, TeamScores> {
    ...
    for line in results.lines() {
       ...
       let entry = scores.entry(team_2_name).or_default(); // checks if it exists and returns a mutable reference, 
       // if not inserts a default value and returns the mutable reference to that.
